...
class PasiastyKwadrat(Kwadrat):
    def sketchPasiasty(self, x, y, paski):
        ...
        for pasek in range(0, paski): 
            ...
            _xLinii_ +=space
class NiebieskiPasiastyKwadrat(PasiastyKwadrat):
    def sketchNiebieskiPasiasty(self,x,y, paski):
        fill(64,224,208)#wypadałoby przestawić kolor później spowrotem na pierwotny, bo inaczej wszystko rysowane następnie bez zmiany kolory będzie w tym kolorze
        PasiastyKwadrat.sketchPasiasty(self, x, y, paski)
class CzerwonyKwadrat(Kwadrat):
    def sketchCzerwony(self,x,y):
        fill(255,0,0)
        Kwadrat.sketch(self, x, y) # po co powtarzać, skoro to już zostało napisane i można tylko wywołać? o to chodzi w dziedziczeniu
